...
建立最小受管理員保護的能力驗證入口，使用與正式功能相同的後端轉接器、資料庫及串流服務；公開世界與下載總開關保持關閉。保存 Git 提交後經 Sites 建立版本與部署，不以本機 workerd 結果代替正式環境。
...
所有必要驗收通過後才設定正式公開下載。Sites 先套用秘密與資料庫遷移，保存／部署對應 Git 完整提交的版本；確認外層公開存取允許匿名，後台仍由自製帳密保護。不得自動發布任何世界。

透過 Sites 取得 realms.jkesbyebye.com 的實際驗證／路由記錄，再在 Bluehost 設定；不猜 CNAME 或 A 目標。確認 DNS、憑證、HTTPS、匿名下載與管理登入均正常，不能把等待中的憑證標成完成。
