...
**狀態**：T001～T023 最小共用核心已完成，可啟動本機驗證入口；68 項本機測試、型別、ESLint 及建置通過。真實 G0 與完整產品尚未完成；以下後續驗收步驟不代表已通過。

使用者於 2026-09-15 明確要求正式網址與所有訪客免登入下載；目前已部署公開設定，正式入口為 `https://realms.jkesbyebye.com`，DNS／HTTPS／匿名實測狀態見 [正式網域紀錄](validation/domain.md)。開啟首頁，按「下載最新世界」及「儲存最新 .mcworld」；每次新工作均重新請求官方 latest。管理員請在正式網域重新登入既有網站帳號，舊網域的登入 Cookie 不移轉，無須因此重新連接 Microsoft。使用者先前回覆尚未匯入，遊戲匯入及內容確認仍待本人完成。
...
7. 使用 T023 最小發布操作確認最新範圍，逐一發布驗收欄位後，從管理員驗證入口下載一份最新；歷史下載待日後獨立啟用及確認。不開啟公開總開關。確認內容對應選擇、記錄大小，其中至少一份由管理員在基岩版匯入並確認世界內容。
...
G0 任何必要項目失敗都阻止 G1／G2 完整產品實作。紀錄失敗、平台限制與需調整決策；不改外部主機、歷史依新範圍另行驗證、不拿模擬串接當真實成功。缺少使用者授權或遊戲匯入確認時，相關項目保持未完成。
...
T092 核對無遺留 G0 發布，由管理員重新明確選取正式欄位並確認最新存檔範圍，才套用外層公開存取及 DOWNLOADS_ENABLED=true。正式網域驗證匿名最新／歷史、自製管理登入、隔離及紀錄，補齊 FR-001／SC-010；全部 FR／SC 均有通過證據才完成正式驗收。失敗立即關閉新公開下載，修正並重驗，不能把等待中的 DNS 或憑證標成完成。
