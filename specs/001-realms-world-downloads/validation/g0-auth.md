...
Sites 版本 9（來源 `374330ab73cc38b937b7aa9fe894a75dac1dc80d`、環境 revision 6）於 2026-09-14 15:34:41.201 UTC 部署成功。使用者原內建瀏覽器重新載入修正版後，按一次「讀取世界」顯示「正在讀取世界…」並完成列表；頁面唯讀工具回報 signedIn=true、connection=connected、loadedWorlds=6、publishedWorlds=1，沒有錯誤。此時真實續期已在前述調查中完成，因此此次部署後觀察驗證一般讀取與進度顯示；新自動接續的多階段、取消及上限由上述回歸驗證，未再次強制刷新有效授權。型別、ESLint 及生產建置均通過。
