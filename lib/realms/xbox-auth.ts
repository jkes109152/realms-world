...
export type XboxStage = "user" | "device" | "title" | "xsts";
export type XboxState = { proofJwk: JsonWebKey; accessToken: string; userToken?: string; deviceToken?: string; titleToken?: string };
...
export async function exchangeXboxOnce(state: XboxState, stage: XboxStage, now: number, fetcher: typeof fetch = fetch): Promise<string | RealmsAuthorization> {
  ...
  return { ownerXuid, userHash: nonempty(claim.uhs, 128), xstsToken: nonempty(token.Token), expiresAt };
}
