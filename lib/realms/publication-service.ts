...
export async function setPublication(db: D1Database, publicId: string, published: boolean, expectedVersion: number, acknowledgeLatest: boolean, now: number) {
  if (!Number.isSafeInteger(expectedVersion) || expectedVersion < 0 || (published && !acknowledgeLatest)) throw new AppError("invalid_request");
  ...
  const results = await db.batch([
    ...
    db.prepare(`UPDATE world_slots SET published=?,publication_scope='latest',publication_version=publication_version+1,updated_at=? WHERE ${target} RETURNING public_id,published,publication_version`).bind(published ? 1 : 0, now, ...values),
  ]);
  ...
  return { worldId: publicId, published, publicationScope: "latest" as const, publicationVersion: expectedVersion + 1 };
}
